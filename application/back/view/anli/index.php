...
<div role="tabpanel" class="tab-pane" id="user" style="display:block;">
	<div class="check-div form-inline row">
				<div class="col-xs-1">
                    <a href="{:url('create')}"><button class="btn btn-yellow btn-xs" data-toggle="modal" data-target="#addUser" id="create">添加案例</button></a>
		</div>
        <div class="col-xs-11">
            <form method="get" action="{:url('index')}" id="searchForm">
                <div class="col-xs-7">
                        <select name="cate_anli_id" style="color:inherit">
                            <option value="">--分类--</option>
<?php foreach($list_cate as $row_cate){?>
    <option value="{$row_cate->id}"  {eq name="Think.get.cate_anli_id"  value="$row_cate->id" }selected{/eq} >{$row_cate->name}</option>
<?php }?>
